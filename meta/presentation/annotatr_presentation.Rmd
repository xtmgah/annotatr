...
- Many high-throughput genomic experiments give genomic regions as results.
    - ChIP-seq gives regions that represent transcription factor (TF) along the genome.
...
- Reading (`read_bed()`)
    - We chose to read files with `readr::read_tsv()` over `base::read.table()` for speedup.
        - **Insert benchmark here.**
    - `BED6` files representing regions transformed into `GenomicRanges`.
- Intersecting (`intersect_annotations()` and `annotate_intersections()`)
    ...
    - Big speedups with `dplyr::bind_rows()` and `dplyr::arrange()` over `base` functions.

## Summarize and Visualize

- Summarize collated intersections with `dplyr::summarize()`, `dplyr::group_by()`.
    - Way faster than `base::split()` + `lapply()`.
...
- We looked at the location of regions of differential methylation in CpG and knownGene annotations.
- The IDH2 mutant prevents methylated CpGs from undergoing the demethylation pathway.
- The NBM is the normal bone marrow.
...
- We implemented `annotatr`, an R package that intersects genomic regions with genomic annotations.
    ...
    - **Fast**.
    - Also a fun opportunity to play with `roxygen2` and `testthat`.
