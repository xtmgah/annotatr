...
A genome must be specified in `intersect_annotations()`, and should naturally match that used in `read_bed()`. The `ignore.strand` parameter is passed directly to the same parameter in `GenomicRanges::findOverlaps()`. If TRUE, overlaps are found regardless of strand information, and if FALSE, overlaps must have the same strand.

The `use.score` parameter of `annotate_intersections()` retrieves information carries along information from the `score` column if TRUE, and not if FALSE.
