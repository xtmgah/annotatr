...
## Annotating Regions with `annotate_regions()`
...
The dataset used in the running example has a class (DMup, DMdown, noDM) and a score (methylation difference between two groups) associated with each region. We will employ all three summarization functions in preparation for visualization.
...
There are three visualization functions -- `visualize_annotation()`, `visualize_score()` and `visualize_name()` -- that are matched with the summarization functions. The visualization functions return an object of type `ggplot` that can be viewed (`print()`) and saved (`ggsave()`).
