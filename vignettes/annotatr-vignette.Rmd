...
The `stranded` parameter in `read_bed()` indicates whether the regions have strand information. The `use.score` parameter incorporates the `score` column from the BED6 file into the `GenomicRanges` object if TRUE, and not if FALSE.
...
A genome must be specified in `annotate_regions()`, and should naturally match that used in `read_bed()`. The `ignore.strand` parameter is passed directly to the same parameter in `GenomicRanges::findOverlaps()`. If TRUE, overlaps are found regardless of strand information, and if FALSE, overlaps must have the same strand. The `use.score` parameter carries along information from the `score` column if TRUE, and not if FALSE.
...
`annotatr` has three visualization functions (`visualize_annotation()`, `visualize_score()` and `visualize_name()`) that are matched with the summarization functions. The visualization functions return an object of type `ggplot` that can be viewed (`print`) and saved (`ggsave`). For maximum flexibility, the user can pass an `annotation_order` parameter into all three visualization functions to ensure the desired order, and to visualize a subset of the annotations. A `data_order` can be passed into `visualize_name()` to order the data classifications as desired, and to visualize a subset of the data classifications.

```{r, fig.align='center', fig.cap='Number of DM regions per annotation.', fig.height=5, fig.width=5, fig.show = 'hold'}
# View the number of regions per annotation.
annots_order = c(
...
visualize_name_cpgs = visualize_name(
  summarized_names = name_summarized_annotations,
  annotation_order = cpgs_order,
